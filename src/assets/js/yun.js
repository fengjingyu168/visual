...
import d3 from 'd3'
// set up SVG for D3
var width  = $(document).width()-583;
var height = $(document).height()-325;
var colors = d3.scale.category10();
var ID;//选择组件的id
var currentNode=[];//选择组件的id
...
var nodes = [
  /*  {id: 0, reflexive: false, image : "img/ecs/ECS.svg",name:"ECS"},
    {id: 1, reflexive: true, image : "img/ecs/云磁盘.svg",name:"云磁盘"},
    {id: 2, reflexive: false, image : "img/ecs/RDS.svg",name:"RDS"}*/
  ],
  lastNodeId = 2,
  links = [
...
function tick() {
  // draw directed edges with proper padding from node centers
    ...
    var deltaX = d.target.x - d.source.x,
        ...
        sourceX = d.source.x + (sourcePadding * normX)+30,
        sourceY = d.source.y + (sourcePadding * normY)+30,
        targetX = d.target.x - (targetPadding * normX)+30,
        targetY = d.target.y - (targetPadding * normY)+30;
  ...
  g.append('svg:image')
    ...
    .on('mousedown', function(d) {
      if(event.button == 2){
          ...
          tools.style.left=event.clientX+40+"px"; 
          tools.style.top=event.clientY-100+"px";//鼠标的坐标啊 
          saveConfigId(d.id);
  ...
  g.append('svg:circle')
    ...
    .attr('fill',"#fff")
     .on('mousedown', function(d) {
      ...
      mousedown_node.x += d3.mouse(this)[0]-50;
      mousedown_node.linesss =!mousedown_node.linesss;
      restart();
    })
...
function mousedown() {
	...
	//  var point = d3.mouse(this),
	      ...
	      node = {id: ++lastNodeId, reflexive: false,image : currentNode[0].image,name:currentNode[0].name};
	  node.x = document.body.scrollLeft+event.clientX-240;
	  node.y = document.body.scrollLeft+event.clientY-115;
	  nodes.push(node);
...
function getNode(ID){
	for(var i=0;i<nodeMsg.length;i++){
		if(nodeMsg[i].ECS_ID == ID){
			currentNode.push(nodeMsg[i]);
		}
	}
...
function mousemove() {
  ...
  if(!mousedown_node) return;
  if(mousedown_node.linesss){
	  //控制划线
//	  var lines = [[mousedown_node.x,mousedown_node.y],[d3.mouse(this)[0],mousedown_node.y],[d3.mouse(this)[0],d3.mouse(this)[1]]];
//	  var linePath = d3.svg.line();
//	  drag_line.attr('d', linePath(lines));
